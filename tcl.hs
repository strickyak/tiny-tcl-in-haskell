...
data Fragment = BareFragment String  -- Unit of substitution.
              ...
              | CmdFragment String
  deriving Show
data TclWord = TclWord [Fragment]          -- Individual args to commands.
  deriving Show
data Sentence = Sentence [TclWord]      -- Individual commands.
  deriving Show

------------------------------------------------
substWords :: [TclWord] -> Act (Either NotOk [String])
substWords [] = good $ []
substWords (w : ws) = do
  x <- substWord w
  x `ifOk` \s -> do
...
substWord :: TclWord -> Action
substWord w = substWord' [] w
  where
    substWord' :: [String] -> TclWord -> Action
    substWord' stringsDone (TclWord []) = good $ concat $ reverse stringsDone
    substWord' stringsDone (TclWord (frag:frags)) = do
      a <- substFragment' frag
      a `ifOk` \s -> substWord' (s:stringsDone) (TclWord frags)
